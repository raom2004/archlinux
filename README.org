...
- [[./script2.sh][script2.sh]] - To setup the new Arch Linux system using =arch-chroot=, for:
  ...
  - sudo
  - keymap
  - Set a preferred shell (e.g. bash, zsh, etc)
...
Once you understand how to install Arch Linux inside a virtual machine, the next natural step is to *install Arch Linux in a Real Machine*. During such process, the scripts must be adapted to the target system, specially the table disk partitioning, hardware, encryption, booting support for preexistent operative systems, user data encryption and the most relevant aspects in Arch Linux, backup an maintenance. 

In this regard, this chapter covers important aspects, such as:
