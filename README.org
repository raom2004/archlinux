...
The problem with this approach is the difficulty to add comments about the purpose of each package. The additional problem about package installation is related to the date of the Arch Linux installation image used, because an old enough can prompt an error about gpg keyring. For the previously mentioned, I started to use a different approach, like this:
