...
Arch Linux encourage users to install packages that fulfill their unique needs, obtaining a lightweight and incredibly powerful system, easy to manage. However, the installation and configuration process can be challenging for beginners, specially when the documentation have not been extensively read. But what to do once a user has completed such process?
