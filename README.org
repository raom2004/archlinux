...
But what to do once a user has completed such process? *you can automatize the Arch Linux installation using the scripts described in this repository*, with tips and recommendations found in the documentation and forums, useful for: 
...
The most easy example to automatize the Arch Linux install, requires almost 3 scripts for specific tasks.
