...
override_git_prompt_colors() {
  ...
  function prompt_callback {
    ...
    echo "${BoldYellow}${PS1}${ResetColor}"
  }

  Time12a="\$(date +%H:%M)"
  GIT_PROMPT_BRANCH="${BoldMagenta}"        # the git branch that is active in the current directory
  GIT_PROMPT_MASTER_BRANCH="${BoldYellow}" # used if the git branch that is active in the current directory is $GIT_PROMPT_MASTER_BRANCHES
